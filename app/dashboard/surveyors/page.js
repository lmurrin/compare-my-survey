"use client";

import { useEffect, useState } from "react";
import DashboardHeading from "@/app/components/dashboard/DashboardHeading";
import { useRouter } from "next/navigation";

export default function DashboardSurveyors() {

  const router = useRouter();

  const [surveyors, setSurveyors] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);


  useEffect(() => {
    const fetchSurveyors = async () => {
      try {
        const response = await fetch("/api/surveyors");
        const data = await response.json();
        setSurveyors(data[0]); // Assuming surveyors are in the first array
      } catch (err) {
        console.error("Failed to fetch surveyors:", err);
        setError("Failed to load surveyors.");
      } finally {
        setLoading(false);
      }
    };

    fetchSurveyors();
  }, []);

  const handleEdit = (surveyor) => {
    // Option 1: Pass only ID via query param (preferred)
    router.push(`/dashboard/surveyors/edit?id=${surveyor.id}`);
  };

  return (
    <>
      <DashboardHeading
        title="Surveyors"
        showEditButton={false}
        showPublishButton={true}
        publishButtonText="Add New"
        publishButtonLink="/dashboard/drafts"
      />

      <div className="px-0 sm:px-6 lg:px-8">
        <div className="mt-8 flow-root">
          <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div className="inline-block min-w-full py-2 align-middle">
              {loading ? (
                <div className="text-gray-500 text-sm">Loading...</div>
              ) : error ? (
                <div className="text-red-500 text-sm">{error}</div>
              ) : (
                <table className="min-w-full divide-y divide-gray-300">
                  <thead>
                    <tr>
                      <th className="py-3 pl-4 pr-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500 sm:pl-0">
                        Company Name
                      </th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">
                        Email
                      </th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">
                        Phone
                      </th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">
                        Address
                      </th>
                      <th className="relative py-3 pl-3 pr-4 sm:pr-0">
                        <span className="sr-only">Edit</span>
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {surveyors.map((surveyor) => (
                      <tr key={surveyor.id}>
                        <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0">
                          {surveyor.companyName}
                        </td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{surveyor.email}</td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{surveyor.phone}</td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{surveyor.address}</td>
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                        <button
                          onClick={() => handleEdit(surveyor)}
                          className="text-indigo-600 hover:text-indigo-900 cursor-pointer"
                        >
                          Edit<span className="sr-only">, {surveyor.companyName}</span>
                        </button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              )}
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
