...
export default function InputPropertyPriceSelect({ onChange, value }) {
  const [selected, setSelected] = useState(value ? propertyPrices.find(p => p.value === value) : propertyPrices[0])
  ...
  return (
    <Listbox value={selected} onChange={handleChange}>
      <Label className="block text-sm/6 font-medium text-gray-900">Property price</Label>
      <div className="relative mt-2">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pl-3 pr-2 text-left text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
          <span className="col-start-1 row-start-1 truncate pr-6">{selected.display}</span>
          <ChevronUpDownIcon
            aria-hidden="true"
        ...
        >
          {propertyPrices.map((price) => (
            ...
            >
              <span className="block truncate font-normal group-data-[selected]:font-semibold">{price.display}</span>
