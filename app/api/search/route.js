...
export async function GET(req) {
  ...
  try {
      ...
      const surveyTypeRecord = await SurveyType.findOne({
          where: {
              name: surveyType
          }
      });
      ...
      const leadPrice = await LeadPrice.findOne({
          where: {
              surveyTypeId: surveyTypeRecord.id
          }
      });
      ...
      if (services.length === 0) {
          return NextResponse.json({
              message: 'No eligible surveyors found'
          }, {
              status: 404
          });
      }
