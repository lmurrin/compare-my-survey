...
export async function POST(request) {
    ...
    try {
        ...
        if (!surveyTypeId || !Array.isArray(surveyors) || surveyors.length === 0) {
            return NextResponse.json({
                error: 'Missing surveyTypeId or surveyors'
            }, {
                status: 400
            });
        }
        ...
        const leadPriceEntry = await LeadPrice.findOne({
            where: {
                surveyTypeId
            }
        });
        ...
        if (!leadPriceEntry) {
            return NextResponse.json({
                error: 'No pricing config found for this survey type'
            }, {
                status: 500
            });
        }
